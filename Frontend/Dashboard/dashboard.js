...
if (!token) {
  ...
  fetch("/api/users/me", {
    ...
    .then((user) => {
      ...
      // --- PERSONALIZED WELCOME MESSAGE LOGIC (FIX) ---
...
function loadRooms(userId) {
  ...
  fetch(`/api/rooms/me?userId=${userId}`, {
    credentials: "include",
    // 🟢 FIX: Added X-Auth-Token for authenticated room list fetching
    headers: {
      "X-Auth-Token": token,
    },
  })
    ...
    .then((rooms) => {
      ...
      if (!rooms || rooms.length === 0) {
        ...
        // 🟢 FIX: Corrected Lottie path to absolute path
        loadLottieAnimation("#no-rooms-message lottie-player", "/NoRooms.json");
        return;
      }
...
function loadMeetings(userId) {
  fetch(`/api/meetings/me?userId=${userId}`, {
    credentials: "include",
    // 🟢 FIX: Added X-Auth-Token for authenticated meeting list fetching
    headers: {
      "X-Auth-Token": token,
    },
  })
...
// CREATE ROOM HANDLING (FIXED LOGIC)
createForm.addEventListener("submit", (e) => {
  ...
  // ❌ REMOVED: Client-side code generation is now handled by the secure backend.
  ...
  fetch("/api/rooms", {
    ...
    headers: {
      "Content-Type": "application/json",
      // 🟢 FIX: Added X-Auth-Token for authenticated room creation
      "X-Auth-Token": token,
    },
    body: JSON.stringify({
      name: name,
      // ❌ REMOVED: code is now generated by the backend
      // ❌ REMOVED: creatorId is now derived from the JWT in the backend
    }),
  })
    .then((res) => {
      if (!res.ok) {
    ...
    .then((room) => {
      ...
      // Show the guaranteed unique code returned by the backend
...
joinForm.addEventListener("submit", (e) => {
  ...
  fetch("/api/rooms/join", {
    ...
    headers: {
      "Content-Type": "application/json",
      // 🟢 FIX: Added X-Auth-Token for authenticated room joining
      "X-Auth-Token": token,
    },
